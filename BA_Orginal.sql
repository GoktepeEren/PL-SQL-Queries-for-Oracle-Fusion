select SIRKET,CUSTOMER_NAME,VERGI_NO,DONEM,count( DISTINCT FATURA_NO) BELGE_SAYISI,sum( TUTAR_TL) TOPLAM_MATRAH from (
select 'POSTED_FLAG',
...
from ra_customer_trx_all rac, ra_customer_trx_lines_all ral, hz_cust_accounts hzc, hz_parties hzp where hzc.cust_account_id=rac.bill_to_customer_id and 
rac.customer_trx_id=ral.customer_trx_id and hzp.party_id=hzc.party_id and 
ral.line_type='LINE'  and (rac.ATTRIBUTE5 not like 'IPT%' or rac.ATTRIBUTE5 is null)
and

 (case when rac.invoice_currency_code ='TRY' then  ral.extended_amount else 
ral.extended_amount*rac.exchange_rate end) <0  and TO_CHAR(RAC.TRX_DATE,'YYYY')||TO_CHAR(RAC.TRX_DATE,'MM') =:P_DONEM  and (rac.ATTRIBUTE14 is null or rac.ATTRIBUTE14='')
union all
SELECT 
...
abs((case when aia.invoice_currency_code<>'TRY' then (aia.INVOICE_AMOUNT-aia.TOTAL_TAX_AMOUNT)* aia.EXCHANGE_RATE  else aia.INVOICE_AMOUNT-aia.TOTAL_TAX_AMOUNT  end ))Fatura_TL,
