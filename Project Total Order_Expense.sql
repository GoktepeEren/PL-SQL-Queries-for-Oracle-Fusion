...
ProjDetail.PeriodStartDate,
ProjDetail.PeriodEndDate,
ProjDetail.AccountTotal3,
ProjDetail.AccountTotal7,
ProjDetail.AccountTotal11
...
(
    ...
    (
        ...
        --13
        NVL(uomt.UNIT_OF_MEASURE, 'Non_UOM') as  LineUnitCode,
        ...
        --40
        NVL(Initcap(projcodemainp.Meaning),'Non-MainProject') as MainProject,
        --17
        Case 
        When Trim(proc.SEGMENT1) = 'DoNotUse-La Romana Common' Then 'La Romana Common Expenses'
        When Trim(proc.SEGMENT1) = 'La Romana Common-Old' Then 'La Romana Common Expenses'
        When Trim(proc.SEGMENT1) is not null Then proc.SEGMENT1
        ...
        --35
        pod.ATTRIBUTE_DATE1 PeriodStartDate,

        --36
        pod.ATTRIBUTE_DATE2 PeriodEndDate,
        ...
        Left Join inv_units_of_measure_vl uomt
        ON uomt.UOM_CODE = pol.UOM_CODE
        Inner Join PO_DISTRIBUTIONS_ALL pod 
            ...
            Inner Join GL_CODE_COMBINATIONS glcode 
                ...
                -- Account3
                Inner Join FND_VS_VALUES_VL valAccoutN3 
                ON valAccoutN3.Value = SUBSTR(glcode.Segment2, 1, 3) and valAccoutN3.ATTRIBUTE_CATEGORY = 'ACM_Account'
                -- Account7
                Inner Join FND_VS_VALUES_VL valAccoutN7
                ON valAccoutN7.Value = SUBSTR(glcode.Segment2, 1, 7) and valAccoutN7.ATTRIBUTE_CATEGORY = 'ACM_Account'
                -- Account11
                Inner Join FND_VS_VALUES_VL valAccoutN11
                ON valAccoutN11.Value = SUBSTR(glcode.Segment2, 1, 11) and valAccoutN11.ATTRIBUTE_CATEGORY = 'ACM_Account'  
            ON pod.CODE_COMBINATION_ID  = glcode.CODE_COMBINATION_ID
            Left Join PJF_PROJECTS_ALL_VL  proc 
                 Left Join PJF_PROJ_ELEMENTS_VL projel
                        Left Join PJT_COLUMN_LKP_VALUES_VL projcodemainp ON projel.TASK_CODE01_ID = projcodemainp.Code_value_Id and projcodemainp.Column_Map_Id = '300000011071779' and projcodemainp.Meaning <> 'TR07'
                    ...
                    On projel.Project_Id = proc.Project_Id and projel.OBJECT_TYPE = 'PJF_STRUCTURES'
            ON proc.Project_Id = pod.PJC_PROJECT_ID
    ...
    (
        ...
        --12
        (gcc.segment2 || ' - ' || accountexp.Description) as AccountDescTotal,
        ...
        --17
        Case 
        When Trim(prj.SEGMENT1) = 'DoNotUse-La Romana Common' Then 'La Romana Common Expenses'
        When Trim(prj.SEGMENT1) = 'La Romana Common-Old' Then 'La Romana Common Expenses'
        When Trim(prj.SEGMENT1) is not null Then prj.SEGMENT1
        Else 'Non-Project'
        End
        as ProjectName,
        ...
        --35
        expdist.PJC_Expenditure_Item_Date PeriodStartDate,

        --36
        expdist.PJC_Expenditure_Item_Date PeriodEndDate,

        --37
        valAccoutN3.Description  || ' ' ||  SUBSTR(gcc.Segment2, 1, 3)   as AccountTotal3,
        
        --38
        valAccoutN7.Description  || ' ' ||  SUBSTR(gcc.Segment2, 1, 7)   as AccountTotal7,
        
        --39
        valAccoutN11.Description || ' ' ||  SUBSTR(gcc.Segment2, 1, 11)  as AccountTotal11
        ...
        Inner Join EXM_EXPENSES expline 
            ...
            Inner Join EXM_EXPENSE_DISTS expdist 
                Inner Join gl_code_combinations gcc
                    ...
                    -- Account3
                    Inner Join FND_VS_VALUES_VL valAccoutN3 
                    ON valAccoutN3.Value = SUBSTR(gcc.Segment2, 1, 3) and valAccoutN3.ATTRIBUTE_CATEGORY = 'ACM_Account'
                    -- Account7
                    Inner Join FND_VS_VALUES_VL valAccoutN7
                    ON valAccoutN7.Value = SUBSTR(gcc.Segment2, 1, 7) and valAccoutN7.ATTRIBUTE_CATEGORY = 'ACM_Account'
                    -- Account11
                    Inner Join FND_VS_VALUES_VL valAccoutN11
                    ON valAccoutN11.Value = SUBSTR(gcc.Segment2, 1, 11) and valAccoutN11.ATTRIBUTE_CATEGORY = 'ACM_Account'           
...
Where 
(ProjDetail.Company IN (:CompanyName) OR 'All' IN (:CompanyName || 'All')) 
and (ProjDetail.MainProject IN (:MainProject) OR 'All' IN (:MainProject || 'All')) 
and (ProjDetail.ProjectName IN (:ProjectX) OR 'All' IN (:ProjectX || 'All')) 
and (ProjDetail.ProjectTaskName IN (:TaskX) OR 'All' IN (:TaskX || 'All')) 
and (ProjDetail.MemberTypeExpd IN (:MemberTypeX) OR 'All' IN (:MemberTypeX || 'All')) 
and (ProjDetail.MemberExpd IN (:MemberX) OR 'All' IN (:MemberX || 'All')) 
and

-- Purchase Analysis Role Control 
(ProjDetail.Company IN 
